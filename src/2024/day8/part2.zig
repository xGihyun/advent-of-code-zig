    ...
    while (map_iter.next()) |entry| {
        ...
        for (entry.value_ptr.items[0 .. entry.value_ptr.items.len - 1], 0..) |cur_pos, i| {
            for (i + 1..entry.value_ptr.items.len) |j| {
                ...
                var cur_antinode_pos = Position{
                    .x = cur_pos.x,
                    .y = cur_pos.y,
                };
                var pair_antinode_pos = Position{
                    .x = pair_pos.x,
                    .y = pair_pos.y,
                };

                while (cur_antinode_pos.x < SIZE and cur_antinode_pos.x >= 0 and
                    cur_antinode_pos.y < SIZE and cur_antinode_pos.y >= 0)
                {
                    ...
                    cur_antinode_pos.x -= cur_vec.x;
                    cur_antinode_pos.y -= cur_vec.y;
                }

                while (pair_antinode_pos.x < SIZE and pair_antinode_pos.x >= 0 and
                    pair_antinode_pos.y < SIZE and pair_antinode_pos.y >= 0)
                {
                    ...
                    pair_antinode_pos.x -= cur_vec.x * -1;
                    pair_antinode_pos.y -= cur_vec.y * -1;
                }
            }
        }
    }
