...
// NOTE:
// My biggest mistake while solving this problem was:
// When performing the simulation with the new obstacle, I put the guard's position
// just before the position where the obstacle was placed.
// The correct way to do it is to either:
// - Simulate starting from the guard's original starting position (inefficient) or;
// - Simulate start from the guard's current position, and only place the obstacle if it
//   the guard hasn't visited that position. 
